...
def norm_distance(d, i1, i2):
  if i1 == i2: return 0
  #af1, af2 = d[i1], d[i2]
  if d[i1] == d[i2]:
    dx = 0
  else:
    dx = float(d[i1] - d[i2]) / max(d[i1], d[i2])
    if dx < 0: dx = -dx
  dist = abs(float(i2 - i1) / len(d))
  dist += dx # abs(af2 - af1)
  return dist

def accum_distr(arr, reverse = False, resolution = 0.01):
  arr.sort(reverse = reverse)
  l = len(arr)
  #arr = list(range(l))
  #dsarr = tools.downsample_even(arr, n)
  out = []
  last = 0
  out.append([arr[0], 1])
  used = {0: 1}
  for i in range(l): #dsarr:
    dist = norm_distance(arr, last, i)
    if dist < resolution: continue # d[i] - last < 0.01: continue
    #if i > 1 and i-1 not in used:
    if i-1 not in used and norm_distance(arr, i-1, i) >= resolution:
      out.append([arr[i-1], i])
      #outf.write(io.tabjoinl(svtype, source, d[i-1], i))
      used[i-1] = 1
    out.append([arr[i], i+1])
    #outf.write(io.tabjoinl(svtype, source, d[i], i+1))
    used[i] = 1
    last = i #d[i]
  if out[-1][1] < l: out.append([arr[i], l])
  #outf.write(io.tabjoinl(svtype, source, 1, l))
  return out
